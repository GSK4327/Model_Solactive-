...
class IndexModelError(ValueError):
    """Raised when the index cannot be calculated."""


def _find_date_column(
    columns: Iterable[object],
) -> object:
    """Find the most likely date column in the CSV file."""
    ...
    # Assume that the first column contains the dates.
    return columns[0]
...
def load_stock_prices(
    ...
    """Load and validate the stock-price data."""
...
def _select_constituents(
    ...
    """
    Select the three stocks with the highest market
    capitalization.

    Since all companies have the same shares outstanding,
    ranking by price is equivalent to ranking by market
    capitalization.
    """
...
def calculate_index_from_prices(
    ...
    # Calculate from the official index start date even
    # when a later output start date is requested.
    calculation_dates = prices.index[
        (prices.index >= INDEX_START_DATE)
        & (prices.index <= requested_end_date)
    ]

    # The first available business date of each month is
    # the monthly rebalance date.
    rebalance_dates = set(
        calculation_dates
        .to_series()
        .groupby(
    ...
    # Units are held constant between monthly rebalances.
    units = pd.Series(
        0.0,
        index=list(UNIVERSE),
        dtype=float,
    ...
    for current_date in calculation_dates:
        ...
        if current_date in rebalance_dates:
            ...
            # Remove all old constituents.
            units[:] = 0.0

            # Establish the new 50%, 25%, 25% basket at
            # the current rebalance-date close.
